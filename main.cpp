...
namespace Memory
{
    ...
    uintptr_t AllocateCodeCave(HANDLE handle, uintptr_t preferred, SIZE_T size)
    {
        for (uintptr_t i = 1; i < 2000; i++)
        {
            ...
            if (!VirtualQueryEx(handle, reinterpret_cast<LPCVOID>(addr), &mbi, sizeof(mbi)))
                continue;
    ...
    bool SetJump(uintptr_t offset, std::vector<int>& bytes)
    {
    ...
    std::vector<int> detour, std::variant<int, float> defaultValue, int varSize = 4)
{
    ...
    if (!Memory::SetJump(hookOffset, hook))
    {
        VirtualFreeEx(processHandle, reinterpret_cast<LPVOID>(detourAddr), 0, MEM_RELEASE);
        return false;
    }
    ...
    if (!Memory::SetJump(returnOffset, detour))
    {
        VirtualFreeEx(processHandle, reinterpret_cast<LPVOID>(detourAddr), 0, MEM_RELEASE);
        return false;
    }
